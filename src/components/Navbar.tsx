...
function Navbar({ handleModal }: { handleModal: () => void }) {
  ...
  const stickNavbar = () => {
    if (window !== undefined) {
      ...
      if (windowHeight > 100) {
        ...
        setSticky('fixed h-24 mt-12 pt-6 ');
      }
    }
  };
  ...
  return (
    <nav
      className={`overflow-x-scroll lg:overflow-hidden z-50 bg-background p-4 text-sm lg:text-xl text-foreground shadow-xl w-full ${sticky} transition-all duration-250 ease-in-out`}
    >
