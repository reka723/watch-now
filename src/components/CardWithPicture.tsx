...
function CardWithPicture({ title, reversed, img }: props) {
    return (
        <Card className="h-fit lg:h-80 w-full lg:w-2/3 flex items-center">
            <CardContent className={`p-6 flex flex-col gap-12 lg:flex-row ${reversed && 'flex-row-reverse'}`}>
                ...
                <img className="object-fill overflow-hidden rounded-sm max-h-svh" src={img} />
            </CardContent>
        </Card>
    )
}
